...
parameters = {
    ...
    "dt": 0.00001,
    "dx": 0.1,
    "chemical": lambda x,y: (-100*np.ones_like(x)*np.linspace(0,1,x.shape[0])).T,
    # "chemical": lambda x,y:-50*((x-0.5)**2+(y-0.5)**2),
    "taxis_strength": 10,
    "n_states":2,
    "state_ODE": lambda C,x,y: np.array([(C-x-y)/parameters["te"],(C-y)/parameters["ta"]]).T,
    "lambda0": 5,
    # "initial_condition": lambda x,y: np.ones_like(x)*np.linspace(0,1,x.shape[0]),
...
ax.plot_surface(ps.X, ps.Y, ps.chemical(ps.X,ps.Y),cmap='coolwarm')
plt.show()